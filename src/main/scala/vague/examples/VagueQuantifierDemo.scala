...
object VagueQuantifierDemo:
  ...
  def basicQuantifierDemo(): Unit =
    ...
    val mostResult = VagueQuantifier.most.evaluateExact(
      employees,
      satisfiedPredicate
    )
    ...
    val aboutHalfResult = VagueQuantifier.aboutHalf.evaluateExact(
      budgetItems,
      spentPredicate
    )
    ...
    val fewResult = VagueQuantifier.few.evaluateExact(
      vulnerabilities,
      criticalPredicate
    )
  ...
  def customQuantifierDemo(): Unit =
    ...
    val result1 = aboutEighty.evaluateExact(testScores, passingPredicate)
    ...
    val result2 = highCompliance.evaluateExact(testScores, compliancePredicate)
    ...
    val result3 = lowError.evaluateExact(testScores, errorPredicate)
  ...
  def knowledgeBaseDemo(): Unit =
    ...
    val manyResult = VagueQuantifier.many.evaluateExact(componentNames, isCritical)
    ...
    val fewResult = VagueQuantifier.few.evaluateExact(riskNames, isHighSeverity)
  ...
  def statisticalSignificanceDemo(): Unit =
    ...
    val exactResult = VagueQuantifier.most.evaluateExact(
      population,
      borderlinePredicate
    )
