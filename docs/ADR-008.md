...
- Consumer projects (e.g. register) need richer domain types (`RiskDomain`, `LeafId`, etc.) — the pipeline must propagate `D` end-to-end without forcing `asInstanceOf` at consumption sites
