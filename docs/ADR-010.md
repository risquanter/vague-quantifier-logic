...
// Query DSL
Query.quantifier(q).over("component").where[D](pred)  // wraps internally
Predicates.inRelation(source, "critical")              // wraps internally
```
...
| `fol/query/Query.scala` | `DomainSpec.Relation(RelationName, Int)`; `over()` and `Predicates` accept `String` |
