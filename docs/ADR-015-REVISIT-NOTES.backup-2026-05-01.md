...
The FOL engine cannot know in advance what JVM type a consumer will use to represent any given sort — Asset might be `String` here, `LeafId` elsewhere, `case class Asset(...)` in a third project. So `Value.raw: Any` is unavoidable as the internal carrier.
...
Register currently uses `String`, `Long`, `Double` as the carriers for Asset, Loss, Probability. These are the *easiest* possible case for ADR-015 — and the named-constant pathway still produces a shape mismatch. A future consumer with a non-primitive Asset carrier (e.g. `LeafId`) would see the same problem at *every* dispatcher arm comparing tree-supplied values against literal-injected ones.
...
- A dedicated VQL plan introduces a symmetric typeclass-driven boundary design (`LiteralParser[A]` + `Extract[A]`) and authors **ADR-016** superseding ADR-015 §1.
...
- **Opaque-type follow-up is strict "after both plans"** — not folded into the rewritten Phase 5b. (Note: register currently has no `LeafId`/`RiskId` opaque types; whether to introduce them is itself an open question deferred to that follow-up.)

Confirmed user answers (2026-05-01):
- VQL version target: `0.11.0-SNAPSHOT` ✓
- Opaque-type timing: strict "after both plans" ✓
- Sequencing: Option 2 as laid out in `register/docs/PLAN-QUERY-NODE-NAME-LITERALS.md` analysis turn ✓

Open questions remaining (need answers before authoring the dedicated VQL plan):
1. **B1 stabilisation strategy:** `ignore`-with-comment vs Option A workaround. Recommendation stands: `ignore`-with-comment.
2. **ADR-016 supersession scope:** supersede only ADR-015 §1, or supersede ADR-015 entirely (rolling §2 content into ADR-016).
3. **Opaque-type necessity in register:** to be evaluated in the post-completion follow-up; not blocking.
