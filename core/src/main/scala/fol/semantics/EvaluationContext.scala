...
import semantics.{Model, Valuation, Domain, FOLSemantics}
...
/** Context for evaluating FOL formulas in a model.
  ...
  *
  * Moved from `semantics` to `fol.semantics` — exclusively consumed by
  * [[fol.semantics.ScopeEvaluator]] and [[fol.bridge.FOLBridge]]; the
  * generic-FOL `semantics` layer has no need to depend on it.
  *
  * Example usage:
  * {{{
...
extension [D](ctx: EvaluationContext[D])
  ...
  /** Evaluate formula with a single domain-element binding.
    ...
    * Used by [[fol.bridge.FOLBridge]] and [[fol.semantics.ScopeEvaluator]] to bind
    * the quantified variable for each element.
    *
    * @param formula  Formula to evaluate
